...
resource "aws_api_gateway_integration" "orchestrator" {
  ...
  depends_on = [aws_api_gateway_method.create_presentation]
}
...
resource "aws_api_gateway_integration" "list_orchestrator" {
  ...
  depends_on = [aws_api_gateway_method.list_presentations]
}
...
# Integration responses are not needed for AWS_PROXY integrations
# The Lambda function handles the response including CORS headers
